...
scc_socket_init(int port)
{
	...
	if(g_scc.g_wsastartup_called == 0) {
		ret = WSAStartup(MAKEWORD(2,0), &wsadata);
		printf("WSAStartup ret: %d\n", ret);
		g_scc.g_wsastartup_called = 1;
	}
#endif
...
scc_socket_open_outgoing(int port, double dcycs)
{
	...
	int requestport = 23; // default telnet
	const char* str = (const char*)&scc_ptr->modem_cmd_str[0];
	char host[1024];
	int l = strlen(str);
	int i ;
	for(i=0;i<l;i++)
	...
	host[i]=0;	
sa_in.sin_port = htons(requestport);
	hostentptr = gethostbyname(host);	// OG Added Cast
	if(hostentptr == 0) {
#ifdef _WIN32
	...
	scc_ptr->sockfd = sockfd;
//	scc_ptr->state = 1;		/* successful socket */
	scc_ptr->state = 0;		//delay state
	scc_delayed_ptr = scc_ptr;
	add_event_entry(dcycs+(5000/16)*DCYCS_IN_16MS , EV_SCC_ENABLE); //1s delay

	scc_socket_make_nonblock(port, dcycs);
	scc_ptr->rdwrfd = scc_ptr->sockfd;
}

void scc_delayed_enable()
{
	if (scc_delayed_ptr)
	{
		printf("enabling scc\n");
		scc_delayed_ptr->state = 1;
		scc_delayed_ptr = NULL;
	}
}
...
scc_socket_close(int port, int full_close, double dcycs)
{
	...
	scc_ptr->socket_state = 0;
	if(g_sim65816.g_serial_modem[port]) {
...
scc_socket_fill_readbuf(int port, int space_left, double dcycs)
{
	...
	if(scc_ptr->socket_state == 0 && g_sim65816.g_serial_modem[port]) {
...
# endif
	...
	if((diff_dcycs > 900.0*1000) && (scc_ptr->modem_plus_mode == 3) &&
				(scc_ptr->socket_state >= 1) &&
				(g_sim65816.g_serial_modem[port] != 0)) {
	...
	while(!done) {
		...
		if(scc_ptr->socket_state < 1 && g_sim65816.g_serial_modem[port]) {
			len = 1;
			...
			if(rdwrfd == -1) {
				if(g_sim65816.g_serial_modem[port]) {
					printf("socket_state: %d, ser_mod: %d, "
						...
						g_sim65816.g_serial_modem[port], rdwrfd);
				}
				scc_ptr->socket_state = 0;
				scc_socket_maybe_open_incoming(port, dcycs);
				return;
...
scc_socket_modem_do_ring(int port, double dcycs)
{
	...
	if(num_rings > 0 && scc_ptr->socket_state == 0) {
		...
		if(diff_dcycs < 2.0*1000*1000 && g_sim65816.g_serial_modem[port]) {
		...
		if(num_rings <= 0) {
			/* decide on answering */
			if(scc_ptr->modem_s0_val || (g_sim65816.g_serial_modem[port]==0)) {
				scc_socket_do_answer(port, dcycs);
			} else {
