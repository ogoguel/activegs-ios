...
system_sound g_system_sounds[] =
{
    { "floppy_eject.wav", 0 },
    { "Spin Up Search 1.wav", 0 },
    { "Spin Up Search 2.wav", 0 },
    { "Search Skip Search 1.wav", 0 },
    { "Search Skip Search 2.wav", 0 },
    { "Search Skip Search 3.wav", 0 },
    { "Search Skip Search 4.wav", 0 },
    { "Search Skip Search 5.wav", 0 },
};
...
#define UPDATE_G_DCYCS_PER_DOC_UPDATE(osc_en)				\
	g_sound.g_dcycs_per_doc_update = (double)((osc_en + 2) * DCYCS_1_MHZ) /	\
		DOC_SCAN_RATE;						\
	g_sound.g_dupd_per_dcyc = 1.0 / g_sound.g_dcycs_per_doc_update;			\
	g_sound.g_drecip_osc_en_plus_2 = 1.0 / (double)(osc_en + 2);
...
doc_log_rout(char *msg, int osc, double dsamps, int etc)
{
	...
	g_doc_log[pos].dtmp2 = g_sound.g_last_sound_play_dsamp;
	g_doc_log[pos].etc = etc;
	if(osc >= 0 && osc < 32) {
		g_doc_log[pos].doc_reg = g_sound.g_doc_regs[osc];
	}
	pos++;
	if(pos >= LEN_DOC_LOG) {
		pos = 0;
...
show_doc_log(void)
{
	...
	fprintf(docfile, "dsamps_now: %f\n",
		(g_sim65816.g_cur_dcycs * g_sound.g_dsamps_per_dcyc) - dsamp_start);
	fprintf(docfile, "g_sound.g_doc_num_osc_en: %d\n", g_sound.g_doc_num_osc_en);
	fclose(docfile);
}
...
sound_init()
{
	...
	g_sound.g_num_c030_fsamps = 0;
	g_sound.g_sound_shm_pos = 0;
	g_sound.g_queued_samps = 0;
	g_sound.g_queued_nonsamps = 0;

	g_sound.doc_sound_ctl = 0;
	g_sound.doc_saved_val = 0;
	g_sound.g_doc_num_osc_en = 1;
	g_sound.g_dcycs_per_doc_update = 1.0;
	g_sound.g_dupd_per_dcyc = 1.0;
	g_sound.g_drecip_osc_en_plus_2 = 1.0 / (double)(1 + 2);

	g_sound.doc_reg_e0 = 0xff;
	//g_sound.g_audio_rate = 0;
	g_sound.g_daudio_rate = 0.0;
	g_sound.g_drecip_audio_rate = 0.0;
	g_sound.g_dsamps_per_dcyc = 0.0;
	g_sound.g_dcycs_per_samp = 0.0;
	g_sound.g_fsamps_per_dcyc = 0.0;

	g_sound.g_doc_vol = 2;

	g_sound.g_last_sound_play_dsamp = 0.0;
...
sound_init_general()
{
	...
	g_sound_shm_addr = x_sound_allocate(size);;

	fflush(stdout);

	x_snd_init(g_sound_shm_addr);
	
	child_sound_loop(-1, -1, g_sound_shm_addr);
...
{
	printf("set audio rate : %d\n",rate);
	g_sound.g_audio_rate = rate;
	g_sound.g_daudio_rate = (rate)*1.0;
	g_sound.g_drecip_audio_rate = 1.0/(rate);
	g_sound.g_dsamps_per_dcyc = ((rate*1.0) / DCYCS_1_MHZ);
	g_sound.g_dcycs_per_samp = (DCYCS_1_MHZ / (rate*1.0));
	g_sound.g_fsamps_per_dcyc = (float)((rate*1.0) / DCYCS_1_MHZ);
}
...
sound_reset(double dcycs)
{
	...
	dsamps = dcycs * g_sound.g_dsamps_per_dcyc;
	for(i = 0; i < 32; i++) {
		doc_write_ctl_reg(i, g_sound.g_doc_regs[i].ctl | 1, dsamps);
		g_sound.doc_reg_e0 = 0xff;	// OG : Don't understand????
		if(g_sound.g_doc_regs[i].has_irq_pending) {
			halt_printf("reset: has_irq[%02x] = %d\n", i,
				g_sound.g_doc_regs[i].has_irq_pending);
		}
		g_sound.g_doc_regs[i].has_irq_pending = 0;
	}
	if(g_sound.g_num_osc_interrupting) {
		halt_printf("reset: num_osc_int:%d\n", g_sound.g_num_osc_interrupting);
	}
	g_sound.g_num_osc_interrupting = 0;
...
void sound_shutdown()
{
	...
	extern void x_sound_free(word32* mem);
	x_sound_free(g_sound_shm_addr);
...
send_sound(int real_samps, int size)
{
	...
	DOC_LOG("send_sound", -1, g_sound.g_last_sound_play_dsamp,
						(real_samps << 30) + size);

#if !defined(DRIVER_LINUX)
	ret = 0;
...
show_c030_state()
{
	show_c030_samps(&(g_sound.g_samp_buf[0]), 100);
}
...
show_c030_samps(int *outptr, int num)
{
	...
	printf("c030_fsamps[]: %d\n", g_sound.g_num_c030_fsamps);

	for(i = 0; i < g_sound.g_num_c030_fsamps+2; i++) {
		printf("%3d: %5.3f\n", i, g_sound.c030_fsamps[i]);
	}
...
sound_play(double dsamps)
{
	...
	g_num_snd_plays++;
	if(g_sound.g_sound_play_depth) {
		halt_printf("Nested sound_play!\n");
	}

	g_sound.g_sound_play_depth++;

	/* calc sample num */

	last_dsamp = g_sound.g_last_sound_play_dsamp;
	...
	outptr_start = &(g_sound.g_samp_buf[0]);
	outptr = outptr_start;
	...
	num = g_sound.g_num_c030_fsamps;

	if(num || ((g_sim65816.g_vbl_count - g_sound.g_last_c030_vbl_count) < 240)) {

		if(num) {
			g_sound.g_last_c030_vbl_count = g_sim65816.g_vbl_count;
		}

		pos = 0;
		outptr = outptr_start;
		c030_state = g_sound.g_c030_state;

		c030_hi_val = ((VAL_C030_BASE + VAL_C030_RANGE)*g_sound.g_doc_vol) >> 4;
		c030_lo_val = (VAL_C030_BASE * g_sound.g_doc_vol) >> 4;

		fc030_range = (float)(((VAL_C030_RANGE) * g_sound.g_doc_vol) >> 4);
		fc030_base = (float)(((VAL_C030_BASE) * g_sound.g_doc_vol) >> 4);
		...
		g_sound.c030_fsamps[num] = (float)(num_samps);
		g_sound.c030_fsamps[num+1] = (float)(num_samps+1);

		ftmp = (float)num_samps;
		/* ensure that all samps are in range */
		for(i = num - 1; i >= 0; i--) {
			if(g_sound.c030_fsamps[i] > ftmp) {
				g_sound.c030_fsamps[i] = ftmp;
			}
		}

		num++;
		fsampnum = g_sound.c030_fsamps[0];
		sampnum = (int)fsampnum;
		fpercent = (float)0.0;
		i = 0;

		while(i < num) {
			next_fsampnum = g_sound.c030_fsamps[i+1];
		...
		g_sound.g_c030_state = !c030_state;
	...
	num_osc_en = g_sound.g_doc_num_osc_en;
	...
	while(!done) {
		...
		for(j = 0; j < num_osc_en; j++) {
			...
			imul = (rptr->vol * g_sound.g_doc_vol);
			off = imul * 128;
			...
			if(imul == 0 || samps_to_do == 0) {
				...
				if(samps_left <= 0) {
					...
					val = g_sound.doc_ram[cur_pos >> SND_PTR_SHIFT];
				}
	...
	g_sound.g_last_sound_play_dsamp = dsamp_now;

	GET_ITIMER(start_time3);

	outptr = outptr_start;

	pos = g_sound.g_sound_shm_pos;
...
# endif
			...
			if(g_sound.g_queued_nonsamps) {
				/* force out old 0 samps */
				send_sound(0, g_sound.g_queued_nonsamps);
				g_sound.g_queued_nonsamps = 0;
			}
	
			if(g_send_sound_to_file) {
				send_sound_to_file(g_sound_shm_addr,
						g_sound.g_sound_shm_pos, num_samps);
			}
	
			g_sound.g_queued_samps += num_samps;
		} else {
			/* move pos */
			pos += num_samps;
			...
			if(g_send_sound_to_file) {
				send_sound_to_file(g_sound.zero_buf, g_sound.g_sound_shm_pos,
					num_samps);
			}
	
			if(g_sound.g_queued_samps) {
				/* force out old non-0 samps */
				send_sound(1, g_sound.g_queued_samps);
				g_sound.g_queued_samps = 0;
			}
	
			g_sound.g_queued_nonsamps += num_samps;
		}

	}

	g_sound.g_sound_shm_pos = pos;


	GET_ITIMER(end_time3);

	g_fvoices += ((float)(samps_played) * (float)(g_sound.g_drecip_audio_rate));

	if(g_sound.g_audio_enable != 0) {
		if(g_sound.g_queued_samps >= (g_sound.g_audio_rate/32)) {
			send_sound(1, g_sound.g_queued_samps);
			g_sound.g_queued_samps = 0;
		}

		if(g_sound.g_queued_nonsamps >= (g_sound.g_audio_rate/32)) {
			send_sound(0, g_sound.g_queued_nonsamps);
			g_sound.g_queued_nonsamps = 0;
		}
	}
...
doc_handle_event(int osc, double dcycs)
{
	...
	g_sound.g_doc_regs[osc].event = 0;
...
doc_sound_end(int osc, int can_repeat, double eff_dsamps, double dsamps)
{
	...
	mode = (ctl >> 1) & 3;
	other_osc = osc ^ 1;
	orptr = &(g_sound.g_doc_regs[other_osc]);
	omode = (orptr->ctl >> 1) & 3;
...
add_sound_irq(int osc)
{
	...
	if(g_sound.g_doc_regs[osc].has_irq_pending) {
		halt_printf("Adding sound_irq for %02x, but irq_p: %d\n", osc,
			g_sound.g_doc_regs[osc].has_irq_pending);
	}

	num_osc_interrupting = g_sound.g_num_osc_interrupting + 1;
	g_sound.g_doc_regs[osc].has_irq_pending = num_osc_interrupting;
	g_sound.g_num_osc_interrupting = num_osc_interrupting;

	add_irq(IRQ_PENDING_DOC);
	if(num_osc_interrupting == 1) {
		g_sound.doc_reg_e0 = 0x00 + (osc << 1);
	}

	DOC_LOG("add_irq", osc, g_cur_dcycs * g_sound.g_dsamps_per_dcyc, 0);
}
...
remove_sound_irq(int osc, int must)
{
	...
	doc_printf("remove irq for osc: %d, has_irq: %d\n",
		osc, g_sound.g_doc_regs[osc].has_irq_pending);

	num_osc_interrupt = g_sound.g_doc_regs[osc].has_irq_pending;
	first = 0;
	if(num_osc_interrupt) {
		g_sound.g_num_osc_interrupting--;
		g_sound.g_doc_regs[osc].has_irq_pending = 0;
		DOC_LOG("rem_irq", osc, g_cur_dcycs * g_sound.g_dsamps_per_dcyc, 0);
		if(g_sound.g_num_osc_interrupting == 0) {
			remove_irq(IRQ_PENDING_DOC);
		}

		first = 0x40 | (g_sound.doc_reg_e0 >> 1);
					/* if none found, then def = no ints */
		for(i = 0; i < g_sound.g_doc_num_osc_en; i++) {
			rptr = &(g_sound.g_doc_regs[i]);
		...
		if(g_sound.doc_reg_e0 != 0xff) {
			halt_printf("remove_sound_irq[%02x]=0, but e0: %02x\n",
				osc, g_sound.doc_reg_e0);
		}
#endif
		if(must) {
			halt_printf("REMOVE_sound_irq[%02x]=0, but e0: %02x\n",
	...
	if(g_sound.doc_reg_e0 & 0x80) {
		for(i = 0; i < 0x20; i++) {
			has_irq_pending = g_sound.g_doc_regs[i].has_irq_pending;
			if(has_irq_pending) {
...
start_sound(int osc, double eff_dsamps, double dsamps)
{
	...
	if(osc >= g_sound.g_doc_num_osc_en) {
		rptr->ctl |= 1;
		return;
	}
...
wave_end_estimate(int osc, double eff_dsamps, double dsamps)
{
	...
	dcycs_per_samp = g_sound.g_dcycs_per_samp;
	...
	ptr1 = &g_sound.doc_ram[pos];

	estimate = 0;
	if(rptr->ctl & 0x08 || g_sound.g_doc_regs[osc ^ 1].ctl & 0x08) {
		estimate = 1;
	}
...
{
	if(g_sound.g_doc_regs[osc].event) {
		g_sound.g_doc_regs[osc].event = 0;
		remove_event_doc(osc);
	}
}
...
doc_write_ctl_reg(int osc, int val, double dsamps)
{
	...
	rptr = &(g_sound.g_doc_regs[osc]);
	old_val = rptr->ctl;
	g_sound.g_doc_saved_ctl = old_val;
	...
	if(new_halt != 0) {
		...
		if(((old_val >> 1) & 3) > 0) {
			...
			g_sound.g_doc_regs[osc].cur_acc = 0;
		}

		g_sound.g_doc_regs[osc].ctl = val;
		g_sound.g_doc_regs[osc].running = 0;
	} else {
...
doc_recalc_sound_parms(int osc, double eff_dcycs, double dsamps)
{
	...
	dtmp1 = dfreq * (DOC_SCAN_RATE * g_sound.g_drecip_audio_rate);
	dacc = (double)(1 << (20 - (17 - sz + res)));
	dacc_recip = (SND_PTR_SHIFT_DBL) / ((double)(1 << 20));
	dtmp1 = dtmp1 * g_sound.g_drecip_osc_en_plus_2 * dacc * dacc_recip;
...
doc_read_c030(double dcycs)
{
	...
	g_sound.c030_fsamps[num] = (float)(dcycs * g_sound.g_dsamps_per_dcyc - 	g_sound.g_last_sound_play_dsamp);
	g_sound.g_num_c030_fsamps = num + 1;
...
doc_read_c03d(double dcycs)
{
	...
	ret = g_sound.doc_saved_val;
	dsamps = dcycs * g_sound.g_dsamps_per_dcyc;

	if(g_sound.doc_sound_ctl & 0x40) {
		/* Read RAM */
		g_sound.doc_saved_val = g_sound.doc_ram[g_moremem.g_c03ef_doc_ptr];
	} else {
		/* Read DOC */
		g_sound.doc_saved_val = 0;

		osc = g_moremem.g_c03ef_doc_ptr & 0x1f;
		type = (g_moremem.g_c03ef_doc_ptr >> 5) & 0x7;
		rptr = &(g_sound.g_doc_regs[osc]);

		switch(type) {
		case 0x0:	/* freq lo */
			g_sound.doc_saved_val = rptr->freq & 0xff;
			break;
		case 0x1:	/* freq hi */
			g_sound.doc_saved_val = rptr->freq >> 8;
			break;
		case 0x2:	/* vol */
			g_sound.doc_saved_val = rptr->vol;
			break;
		case 0x3:	/* data register */
			/* HACK: make this call sound_play sometimes */
			g_sound.doc_saved_val = rptr->last_samp_val;
			break;
		case 0x4:	/* wave ptr register */
			g_sound.doc_saved_val = rptr->waveptr;
			break;
		case 0x5:	/* control register */
			g_sound.doc_saved_val = rptr->ctl;
			break;
		case 0x6:	/* control register */
			g_sound.doc_saved_val = rptr->wavesize;
			break;
		case 0x7:	/* 0xe0-0xff */
			switch(osc) {
			case 0x00:	/* 0xe0 */
				g_sound.doc_saved_val = g_sound.doc_reg_e0;
				doc_printf("Reading doc 0xe0, ret: %02x\n",
								g_sound.doc_saved_val);

				/* Clear IRQ on read of e0, if any irq pend */
				if((g_sound.doc_reg_e0 & 0x80) == 0) {
					remove_sound_irq(g_sound.doc_reg_e0 >> 1, 1);
				}
				break;
			case 0x01:	/* 0xe1 */
				g_sound.doc_saved_val = (g_sound.g_doc_num_osc_en - 1) << 1;
				break;
			case 0x02:	/* 0xe2 */
				g_sound.doc_saved_val = 0x80;
#if 0
			...
			default:
				...
				halt_printf("Reading bad doc_reg[%04x]: %02x\n",
					g_moremem.g_c03ef_doc_ptr, g_sound.doc_saved_val);
			}
			break;
		default:
			g_sound.doc_saved_val = 0;
	...
	doc_printf("read c03d, doc_ptr: %04x, ret: %02x, saved: %02x\n",
		g_moremem.g_c03ef_doc_ptr, ret, g_sound.doc_saved_val);

	DOC_LOG("read c03d", -1, dsamps, (g_moremem.g_c03ef_doc_ptr << 16) +
			(g_sound.doc_saved_val << 8) + ret);

	if(g_sound.doc_sound_ctl & 0x20) {
		g_moremem.g_c03ef_doc_ptr = (g_moremem.g_c03ef_doc_ptr + 1) & 0xffff;
	}
...
doc_write_c03c(int val, double dcycs)
{
	...
	vol = val & 0xf;
	if(g_sound.g_doc_vol != vol) {
		/* don't bother playing sound..wait till next update */
		/* sound_play(dcycs); */

		g_sound.g_doc_vol = vol;
		doc_printf("Setting doc vol to 0x%x at %f\n",
			vol, dcycs);
	}
	DOC_LOG("c03c write", -1, dcycs * g_sound.g_dsamps_per_dcyc, val);

	g_sound.doc_sound_ctl = val;
}
...
doc_write_c03d(int val, double dcycs)
{
	...
	doc_printf("write c03d, doc_ptr: %04x, val: %02x\n",
		g_moremem.g_c03ef_doc_ptr, val);

	DOC_LOG("write c03d", -1, dsamps, (g_moremem.g_c03ef_doc_ptr << 16) + val);

	if(g_sound.doc_sound_ctl & 0x40) {
		/* RAM */
		g_sound.doc_ram[g_moremem.g_c03ef_doc_ptr] = val;
	} else {
		/* DOC */
		osc = g_moremem.g_c03ef_doc_ptr & 0x1f;
		type = (g_moremem.g_c03ef_doc_ptr >> 5) & 0x7;
		...
		if((ctl & 1) == 0) {
			if(type < 2 || type == 4 || type == 6) {
				halt_printf("Osc %d is running, old ctl: %02x, "
					...
					osc, ctl, g_moremem.g_c03ef_doc_ptr & 0xff, val);
			}
		}
#endif
		...
		case 0x7:	/* 0xe0-0xff */
			...
			case 0x00:	/* 0xe0 */
				doc_printf("writing doc 0xe0 with %02x, "
					"was:%02x\n", val, g_sound.doc_reg_e0);
#if 0
				if(val != g_sound.doc_reg_e0) {
			...
			case 0x01:	/* 0xe1 */
				...
				/*   are disabled */
				for(i = g_sound.g_doc_num_osc_en; i < 0x20; i++) {
					doc_write_ctl_reg(i,
						g_sound.g_doc_regs[i].ctl | 1, dsamps);
				}
...
doc_show_ensoniq_state(int osc)
{
	...
	printf("c03c doc_sound_ctl: %02x, g_sound.doc_saved_val: %02x\n",
		g_sound.doc_sound_ctl, g_sound.doc_saved_val);
	printf("doc_ptr: %04x,    num_osc_en: %02x, e0: %02x\n",
		g_moremem.g_c03ef_doc_ptr, g_sound.g_doc_num_osc_en, g_sound.doc_reg_e0);

	for(i = 0; i < 32; i += 8) {
		printf("irqp: %02x: %04x %04x %04x %04x %04x %04x %04x %04x\n",
			i,
			g_sound.g_doc_regs[i].has_irq_pending,
			g_sound.g_doc_regs[i + 1].has_irq_pending,
			g_sound.g_doc_regs[i + 2].has_irq_pending,
			g_sound.g_doc_regs[i + 3].has_irq_pending,
			g_sound.g_doc_regs[i + 4].has_irq_pending,
			g_sound.g_doc_regs[i + 5].has_irq_pending,
			g_sound.g_doc_regs[i + 6].has_irq_pending,
			g_sound.g_doc_regs[i + 7].has_irq_pending);
	}

	for(i = 0; i < 32; i += 8) {
		printf("freq: %02x: %04x %04x %04x %04x %04x %04x %04x %04x\n",
			i,
			g_sound.g_doc_regs[i].freq, g_sound.g_doc_regs[i + 1].freq,
			g_sound.g_doc_regs[i + 2].freq, g_sound.g_doc_regs[i + 3].freq,
			g_sound.g_doc_regs[i + 4].freq, g_sound.g_doc_regs[i + 5].freq,
			g_sound.g_doc_regs[i + 6].freq, g_sound.g_doc_regs[i + 7].freq);
	}

	for(i = 0; i < 32; i += 8) {
		printf("vol: %02x: %02x %02x %02x %02x %02x %02x %02x %02x\n",
			i,
			g_sound.g_doc_regs[i].vol, g_sound.g_doc_regs[i + 1].vol,
			g_sound.g_doc_regs[i + 2].vol, g_sound.g_doc_regs[i + 3].vol,
			g_sound.g_doc_regs[i + 4].vol, g_sound.g_doc_regs[i + 5].vol,
			g_sound.g_doc_regs[i + 6].vol, g_sound.g_doc_regs[i + 6].vol);
	}

	for(i = 0; i < 32; i += 8) {
		printf("wptr: %02x: %02x %02x %02x %02x %02x %02x %02x %02x\n",
			i,
			g_sound.g_doc_regs[i].waveptr, g_sound.g_doc_regs[i + 1].waveptr,
			g_sound.g_doc_regs[i + 2].waveptr, g_sound.g_doc_regs[i + 3].waveptr,
			g_sound.g_doc_regs[i + 4].waveptr, g_sound.g_doc_regs[i + 5].waveptr,
			g_sound.g_doc_regs[i + 6].waveptr, g_sound.g_doc_regs[i + 7].waveptr);
	}

	for(i = 0; i < 32; i += 8) {
		printf("ctl: %02x: %02x %02x %02x %02x %02x %02x %02x %02x\n",
			i,
			g_sound.g_doc_regs[i].ctl, g_sound.g_doc_regs[i + 1].ctl,
			g_sound.g_doc_regs[i + 2].ctl, g_sound.g_doc_regs[i + 3].ctl,
			g_sound.g_doc_regs[i + 4].ctl, g_sound.g_doc_regs[i + 5].ctl,
			g_sound.g_doc_regs[i + 6].ctl, g_sound.g_doc_regs[i + 7].ctl);
	}

	for(i = 0; i < 32; i += 8) {
		printf("wsize: %02x: %02x %02x %02x %02x %02x %02x %02x %02x\n",
			i,
			g_sound.g_doc_regs[i].wavesize, g_sound.g_doc_regs[i + 1].wavesize,
			g_sound.g_doc_regs[i + 2].wavesize, g_sound.g_doc_regs[i + 3].wavesize,
			g_sound.g_doc_regs[i + 4].wavesize, g_sound.g_doc_regs[i + 5].wavesize,
			g_sound.g_doc_regs[i + 6].wavesize, g_sound.g_doc_regs[i + 7].wavesize);
	}
#ifndef ACTIVEGS
	show_doc_log();
#endif
...
extern void x_play_sound(enum_sound _hSound);
void play_sound(enum_sound _hSound)
{
    if ( (_hSound == SOUND_EJECTION) && (g_driver.x_notify_eject !=NULL) )
        g_driver.x_notify_eject();    

	 x_play_sound(_hSound);
